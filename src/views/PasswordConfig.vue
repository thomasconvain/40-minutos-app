  ...
  <div>
    ...
    <div class="card bg-base-100 border border-base-600 mt-6">
      <div class="card-body">
        ...
        <form @submit.prevent="handleResetPassword" v-if="!successMessage && !isVerifying">
    ...
    <!-- Depuración del código OOB -->
    <div v-if="process.env.NODE_ENV === 'development'" class="mt-4 p-3 bg-gray-100 rounded text-xs">
      <p><strong>Información de depuración:</strong></p>
      <p>OOB Code: {{ oobCode || 'No disponible' }}</p>
      <p>Email verificado: {{ email || 'No disponible' }}</p>
    </div>
  </div>
</template>
...
onMounted(async () => {
  try {
    ...
    console.log('Verificando código de restablecimiento:', oobCode.value);
...
const handleResetPassword = async () => {
  ...
  try {
    ...
    try {
      ...
      // Si hay error en Firestore, lo registramos pero no fallamos todo el proceso

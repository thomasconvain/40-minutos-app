    ...
    <section v-for="event in events" :key="event.id" class="py-8">
      <div class="mx-auto grid max-w-screen-xl rounded-lg bg-black p-8 md:p-8 lg:grid-cols-12 lg:gap-8 lg:p-16 xl:gap-16 background-circle">
        <div class="me-auto place-self-center lg:col-span-7">
          ...
          <p v-if="event.isOver" class="mb-6 text-gray-100">Evento terminado. Si aún no realizaste tu aporte puedes hacerlo haciendo click en el siguiente botón.</p>
          <p v-else-if="event.isFreeEntrance" class="mb-6 text-gray-100">{{ event.place }}</p>
          <div class="flex flex-wrap gap-3">
            ...
            >
              <span>{{event.isOver ? 'Realizar mi aporte' : 'Ya estoy inscrito'}}</span>
            </button>
          </div>
        </div>
      </div>
...
const convertTimestamp = (timestamp) => {
  ...
  return format(timestamp.toDate(), "EEEE dd 'de' MMMM '|' HH.mm 'hrs'", { locale: es })
}
